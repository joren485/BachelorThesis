...
This list is kept intentionally short and uses only Unix shell commands that can be run manually, to make it easy and quick to use. \autoref{section:tools} describes tools that might be able to automate certain enumeration or exploitation. These are however not necessary to use this checklist.

\medskip

The first steps (\autoref{section:checklist-detection}) are meant to see if we are running inside a container. When we know whether we are inside a container, we can look for vulnerabilities inside the container (see \autoref{section:checklist-vulnerabilities-container}) or on the host (see \autoref{section:checklist-vulnerabilities-host}). For more information about each step, please see the referred section.
...
\begin{itemize}
    ...
    Execute ``\lstinline{ls -l /var/run/docker.sock}'' to see the which owner and group of \lstinline{/var/run/docker.sock} and which users have read and write access to it. The owner and every user in the group is allowed to use Docker.
